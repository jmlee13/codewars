...
function piecesValue(arr, s) {
  let whiteSum = 9*arr.flat().filter((arr) => arr.includes('w-queen')).length + (5*arr.flat().filter((arr) => arr.includes('w-rook')).length) + (3*arr.flat().filter((arr) => arr.includes('w-knight') || arr.includes('w-bishop')).length) + (arr.flat().filter((arr) => arr.includes('w-pawn')).length)
  let blackSum = (9*arr.flat().filter((arr) => arr.includes('b-queen')).length + (5*arr.flat().filter((arr) => arr.includes('b-rook')).length) + (3*arr.flat().filter((arr) => arr.includes('b-knight') || arr.includes('b-bishop')).length) + (arr.flat().filter((arr) => arr.includes('b-pawn')).length)) 
  if (s == 'white'){
    return whiteSum
  }
  else {
